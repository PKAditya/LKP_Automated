...
install_lkp() {
	...
	if ! yes | $lkp_cmd install; then
		capture_error "Couldn't install lkp properly, need manual attention"
	fi
}
...
echo "    2. Only Install LKP, I will run the test cases manually later"
...
if command -v lkp; then
	...
	if [[ $lkp_install_status -eq -1 ]]; then
		...
		# if test_lkp is success, it will change the lkp_install_status value to 0, revert back the value to -1 for later hecks
		lkp_install_status="-1"
	...
	if [[ $lkp_install_status -eq -1 ]]; then
		...
		# if test_lkp is success, it will change the lkp_install_status value to 0, revert back the value to -1 for later he
cks
		lkp_install_status="-1"
